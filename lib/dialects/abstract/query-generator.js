...
class QueryGenerator {
  ...
  */
  quote(collection, parent, connector) {
    ...
    const collectionHasModelAttribute = collection._modelAttribute;
    const collectionIsSequelizeMethod = collection instanceof Utils.SequelizeMethod;
    const collectionIsRawObject = _.isPlainObject(collection) && collection.raw;
    ...
    if (collectionIsArray) {
      ...
      collection.forEach((item, index) => {
        ...
        const previousItemIsAssociation = previous && previous instanceof Association;
        const noPreviousItem = !previous;
        const hasParent = parent !== undefined;
        if (noPreviousItem && hasParent) {
          previousModel = parent;
        } else if (previousItemIsAssociation) {
          previousAssociation = previous;
          previousModel = previous.target;
        }

        // if the previous item is a model, then attempt getting an association
        const previousItemIsModel = previousModel && previousModel.prototype instanceof Model;
        if (previousItemIsModel) {
          let model;
          let as;

          const currentItemFunctionIsModel = typeof item === 'function' && item.prototype instanceof Model;
          const currentItemObjectIsModel = _.isPlainObject(item) && item.model && item.model.prototype instanceof Model;
          if (currentItemFunctionIsModel) {
            // set
            model = item;
          } else if (currentItemObjectIsModel) {
            // set
            model = item.model;
            as = item.as;
          }
          ...
          if (model) {
            ...
            const itemIsAssociation = item instanceof Association;
            if (!itemIsAssociation) {
        ...
        if (itemIsString) {
          ...
          const itemIsOrder = index > 0 && orderIndex !== -1;
          if (itemIsOrder) {
            item = this.sequelize.literal(` ${validOrderOptions[orderIndex]}`);
          } else if (previousItemIsModel) {
            const itemIsJsonAttribute = item.includes('.') && previousModel.rawAttributes !== undefined;
      ...
      return this.buildSqlForArray(collection, connector, parent);
    }

    if (collectionHasModelAttribute) {
      return `${this.quoteTable(collection.Model.name)}.${this.quoteIdentifier(collection.fieldName)}`;
    }

    if (collectionIsSequelizeMethod) {
      return this.handleSequelizeMethod(collection);
    }

    if (collectionIsRawObject) {
  ...
  buildSqlForArray(collection, connector, parent) {
